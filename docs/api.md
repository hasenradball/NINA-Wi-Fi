...
#### Description
Initializes the NINA-Wi-Fi library's network settings and provides the current status.
...
Initializes the NINA-Wi-Fi library in Access Point (AP) mode. Other WiFi devices will be able to discover and connect to the created Access Point.
...
Initializes the NINA-Wi-Fi library's network settings for a common WPA2 Enterprise network with username and password authentication (PEAP/MSCHAPv2).
